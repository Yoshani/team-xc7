...
SYSTEM_PROMPT = (
    ...
    "(2) Cover standard NFR categories comprehensively where relevant: performance, scalability, reliability/availability, security, privacy, usability/accessibility, maintainability, observability, portability, compatibility, compliance/regulatory, and cost/efficiency. "
...
class NFRGenerator:
    ...
    def generate(self, functional_requirements: List[str], domain: Optional[str] = None) -> Dict[str, Any]:
        fr_objs = [{"id": f"FR-{i+1}", "text": fr.strip()} for i, fr in enumerate(functional_requirements) if fr.strip()]
        if not fr_objs:
